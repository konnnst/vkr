...
def main():
    ...
    if get_config_from_image:
        config = ConfigManager.get_config_by_image(img_path)
    else:
    ...
    blocks, dilated_blocks = find_text_blocks(blocks_morph, config)
    lines, dilated_lines = find_lines(blocks, lines_morph, config)
    chars = find_chars(lines, chars_morph)
