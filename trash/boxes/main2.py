...
IMAGE_PATH = "page.jpg"   # константа с путём к изображению


def choose_best_binary(binary_a, binary_b, img_area):
    """
    Выбираем бинарное изображение, на котором больше "разумных" контуров:
    не слишком маленьких и не занимающих почти всю страницу.
    """
    def count_good_components(bin_img):
        ...
        for cnt in contours:
            ...
            if area < 30:
                continue
            if area > 0.2 * img_area:  # отсечь очень большие объекты
                continue
            count += 1
        return count
    ...
    # Берём тот вариант, где больше подходящих компонентов
...
def preprocess_image(img_bgr, show_steps=True):
    ...
    # 4. Otsu-бинаризация (текст и фон пока неизвестно какой полярности)
    ...
    # Два варианта:
    # - binary_raw
    # - инвертированное (255 - binary_raw)
    h, w = binary_raw.shape[:2]
    img_area = h * w
    binary_inv = 255 - binary_raw

    # 5. Выбираем лучший по количеству "разумных" компонентов
    binary_chosen = choose_best_binary(binary_raw, binary_inv, img_area)

    # 6. Лёгкая морфология (OPEN) для удаления мелких шумов,
    #    без сильного склеивания символов
    ...
    if show_steps:
        ...
        plt.subplot(2, 3, 6)
        plt.title("Выбранное + MORPH_OPEN")
...
def get_character_boxes(binary_img):
    ...
    for cnt in contours:
        ...
        # 1) отбрасываем совсем мелкий мусор
        if area < 40:
            continue

        # 2) отбрасываем очень большие области (большая часть страницы)
...
def draw_boxes(img_bgr, boxes):
    ...
    for (x, y, w, h) in boxes:
        cv2.rectangle(img_out, (x, y), (x + w, y + h), (100, 255, 0), 4)
    return img_out
...
def main():
    ...
    # Предобработка + показ промежуточных шагов
